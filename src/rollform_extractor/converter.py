...
def _run_oda(executable: Path, source: Path, staged: Path) -> None:
    ...
    raise ConversionUnavailableError("; ".join(failures) or ASCII_DXF_INSTRUCTIONS)


def _run_libredwg(executable: Path, source: Path, staged: Path) -> None:
    result = subprocess.run(
        [str(executable), "-o", str(staged), str(source)],
        capture_output=True,
        text=True,
        check=False,
    )
    if result.returncode != 0 or not staged.exists():
        raise ConversionUnavailableError(
            (result.stderr or result.stdout or ASCII_DXF_INSTRUCTIONS).strip()
        )
