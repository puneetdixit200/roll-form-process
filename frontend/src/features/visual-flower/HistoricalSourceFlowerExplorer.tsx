...
import { getHistoricalFlower } from "./api";

type SourcePass = { pass_id: string; inferred_order?: number; display_order?: number; width?: number; height?: number; geometry?: { points?: number[][] }; points?: number[][]; shape_vector?: number[]; roller_roles?: Array<{ role: string; designs: Array<{ design_id: string; geometry_revision_id?: string | null; confirmation_status?: string }> }> };
type SourceFlower = { flower_id: string; station_count: number; topology?: string; passes: SourcePass[]; private_paths_redacted: boolean };
...
export function HistoricalSourceFlowerExplorer({ flowerId, passId, generatedStation, onBack }: { flowerId: string; passId: string; generatedStation: number; onBack: () => void }) {
  ...
  useEffect(() => { getHistoricalFlower(flowerId).then((result) => { setFlower(result as SourceFlower); setIndex(Math.max(0, (result.passes ?? []).findIndex((item) => item.pass_id === passId))); }).catch(() => setFlower(null)); }, [flowerId, passId]);
  ...
  const current = flower?.passes[index];
  ...
  return <section className="historical-source-explorer" aria-label="Historical source flower explorer">
    ...
    {!flower ? <p>Historical source could not be loaded.</p> : <>
      ...
      <h4>Historical roller design evidence</h4>{current?.roller_roles?.length ? current.roller_roles.map((role) => <div key={role.role}><strong>{role.role}</strong>{role.designs.map((design) => <span key={`${design.design_id}-${design.geometry_revision_id}`} className="evidence-chip">{design.design_id}{design.geometry_revision_id ? ` / ${design.geometry_revision_id}` : ""} · {design.confirmation_status ?? "UNCONFIRMED"}</span>)}</div>) : <p>No reviewed roller design evidence is recorded for this source pass.</p>}
    </>}
  </section>;
}
