...
export const getVisualImportProfiles = (importId: string) => request<Array<{ profile_id: string; profile?: VisualProfile; open_closed: string; entity_count: number; aspect_ratio: number | null; warnings: string[]; thumbnail_svg: string }>>(`/api/visual-flower/imports/${encodeURIComponent(importId)}/profiles`);
export const useVisualImportProfile = (importId: string, profileId: string) => request<{ target_id: string; profile: VisualProfile }>(`/api/visual-flower/imports/${encodeURIComponent(importId)}/profiles/${encodeURIComponent(profileId)}/use`, { method: "POST" });
export function visualExportUrl(candidateId: string, artifact: string): string { return `${API_ROOT}/api/visual-flower/candidates/${encodeURIComponent(candidateId)}/export/${artifact}`; }
