...
type Candidate = { profile_id: string; candidate_kind?: string; source_handles?: string[] };
...
export function CadDrawingCanvas({ preview, candidates, selectedId, onSelect }: { preview: CadDrawingPreview; candidates: Candidate[]; selectedId: string | null; onSelect: (id: string) => void }) {
  ...
  const [showReferenceEntities, setShowReferenceEntities] = useState(false);
  ...
  const selectedCandidate = candidates.find((item) => item.profile_id === selectedId);
  const selectedOverlay = preview.candidate_overlays?.find((item) => item.candidate_id === selectedId);
  ...
  return <div className="cad-drawing-preview">
    <div className="visual-controls"><button type="button" onClick={() => setZoom((value) => Math.min(8, value * 1.25))}>Zoom +</button><button type="button" onClick={() => setZoom((value) => Math.max(.25, value / 1.25))}>Zoom −</button><button type="button" onClick={() => fit()}>Fit drawing</button><button type="button" disabled={!selectedId} onClick={() => fit(selectedCandidate?.candidate_kind === "DERIVED_CENTERLINE" && selectedOverlay ? selectedOverlay.primitives : preview.primitives.filter((item) => selectedHandles.has(item.source_handle)))}>Fit selected</button><label><input type="checkbox" checked={showReferenceEntities} onChange={(event) => setShowReferenceEntities(event.target.checked)} /> Show reference points</label><span>Pan: drag canvas</span></div>
    <svg data-testid="cad-drawing-canvas" role="img" aria-label="Imported CAD drawing" viewBox={viewBox} onWheel={(event) => { event.preventDefault(); setZoom((value) => Math.max(.25, Math.min(8, value * (event.deltaY < 0 ? 1.1 : .9)))); }} onPointerDown={beginPan} onPointerMove={movePan} onPointerUp={endPan} onPointerCancel={endPan}>
      {preview.primitives.filter((item) => layers[item.layer] !== false).map((item) => { const d = primitivePath(item, flip); if (!d) return null; const owner = candidates.find((candidate) => candidate.source_handles?.includes(item.source_handle)); const active = selectedHandles.has(item.source_handle) && selectedCandidate?.candidate_kind !== "DERIVED_CENTERLINE"; const hover = hoveredHandles.has(item.source_handle) && selectedCandidate?.candidate_kind !== "DERIVED_CENTERLINE"; return <path key={item.primitive_id} d={d} fill="none" stroke={active ? "#d66f00" : hover ? "#155783" : owner ? "#4d78a8" : "#aab6c1"} strokeWidth={active ? 1.8 : hover ? 1.3 : .7} opacity={active || hover || !owner ? 1 : .35} onPointerEnter={() => owner && setHoveredId(owner.profile_id)} onPointerLeave={() => setHoveredId(null)} onClick={(event) => { event.stopPropagation(); if (owner) onSelect(owner.profile_id); }} />; })}
      {(preview.candidate_overlays ?? []).map((overlay) => overlay.primitives.map((item) => { const d = primitivePath(item, flip); if (!d) return null; const active = overlay.candidate_id === selectedId; const hover = overlay.candidate_id === hoveredId; return <path key={item.primitive_id} d={d} fill="none" stroke={active ? "#d66f00" : hover ? "#155783" : "#8c5fbf"} strokeWidth={active ? 2.4 : hover ? 1.8 : 1.1} opacity={active || hover ? 1 : .8} onPointerEnter={() => setHoveredId(overlay.candidate_id)} onPointerLeave={() => setHoveredId(null)} onClick={(event) => { event.stopPropagation(); onSelect(overlay.candidate_id); }} />; }))}
      {showReferenceEntities && (preview.reference_entities ?? []).filter((item) => layers[item.layer] !== false).map((item) => <circle key={item.primitive_id} cx={item.position[0]} cy={-item.position[1]} r={Math.max(b.width, b.height) * .006} fill="#7b61a8" opacity=".75" aria-label="Reference entity" />)}
    </svg>
    <div className="cad-preview-legend"><span><i className="legend-source" />Source boundary</span><span><i className="legend-derived" />Derived centerline</span><span><i className="legend-reference" />Reference points</span></div>
    <div className="cad-preview-meta"><span>Source entities: {preview.source_entity_count ?? preview.modelspace_entity_count}</span><span>Forming geometry: {preview.forming_geometry_entity_count ?? preview.supported_primitive_count}</span><span>Preview primitives: {preview.supported_primitive_count}</span><span>Reference entities: {preview.reference_entity_count ?? 0}</span><span>Profile candidates: {candidates.length}</span><span>Units: {preview.units ?? "unknown"}</span></div>
